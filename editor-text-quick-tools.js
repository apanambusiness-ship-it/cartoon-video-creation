(()=>{const $=s=>document.querySelector(s),stage=$('#stage');if(!stage)return;let active=null;const textEls=()=>[...stage.querySelectorAll('.element.selected')].filter(e=>e.dataset.text!=null);function need(){let a=textEls();if(!a.length&&active)a=[active];if(!a.length)alert('पहले poster में text पर click करें।');return a}function save(){window.APANAM_PROJECT?.save?.()}function heading(){need().forEach(e=>{e.style.fontWeight='700';e.style.fontSize='56px';e.style.lineHeight='1.1';e.style.letterSpacing='-0.5px'});save()}function price(){need().forEach(e=>{e.style.fontWeight='800';e.style.fontSize='48px';e.style.lineHeight='1';e.style.letterSpacing='0';e.style.fontVariantNumeric='tabular-nums'});save()}function readable(){need().forEach(e=>{e.style.fontWeight='500';e.style.fontSize='30px';e.style.lineHeight='1.35';e.style.letterSpacing='0.2px';e.style.maxWidth='85%';e.style.overflowWrap='break-word'});save()}
const css=document.createElement('style');css.textContent=`#apanamTextPanel{margin:10px 0;padding:12px;border:1px solid #ddd6fe;border-radius:12px;background:#faf8ff}#apanamTextPanel input,#apanamTextPanel select{width:100%;padding:8px;margin:4px 0 8px;border:1px solid #ccd3dd;border-radius:8px}#apanamTextPanel .fmt{display:flex;gap:6px}#apanamTextPanel .fmt button{flex:1}#apanamTextColorBtn{width:100%;display:flex;justify-content:space-between;align-items:center;margin-top:8px}#apanamSwatch{width:34px;height:24px;border-radius:6px;background:#6d28d9}#apanamPalette{display:none;position:fixed;z-index:999999;left:270px;top:145px;width:310px;padding:16px;background:white;border:2px solid #6d28d9;border-radius:14px;box-shadow:0 20px 60px #0004}#apanamPalette .ph{display:flex;justify-content:space-between;align-items:center;font-size:18px;font-weight:800;margin-bottom:12px;cursor:move;user-select:none;touch-action:none}#apanamPalette .ph button{cursor:pointer;flex:0 0 30px!important;width:30px!important;height:30px!important;min-width:30px!important;min-height:30px!important;padding:0!important;position:relative!important;z-index:5!important}.ph{position:relative!important;z-index:5!important;min-height:32px!important}.grid{position:relative!important;z-index:1!important;clear:both!important}#apanamPalette .grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}#apanamPalette .pc{width:42px;height:42px;min-height:42px;padding:0;border-radius:50%;border:2px solid #e5e7eb}#apanamPalette .custom{margin-top:14px}#apanamPalette .custom input{width:100%;height:42px}#stage .element[data-text]{pointer-events:auto!important;cursor:pointer!important}@media(max-width:700px){#apanamPalette{left:5%;top:90px;width:90%}}`;document.head.appendChild(css);
