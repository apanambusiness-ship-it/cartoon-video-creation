...
const pricing = ["₹10 registration", "₹15 basic AI poster", "₹25 festival/personal poster", "₹35 political poster", "₹99+ manual custom design"];
const tools: Feature[] = [
...
export default function AdminControls({ initial }: { initial: Record<string, boolean> }) {
  ...
  return <section className="admin-controls"><p className="eyebrow">ADMIN CONTROLS</p><h2>Tool availability</h2><p>Only your admin account can change these settings. Paid tools are off until you activate billing.</p><div className="admin-pricing">{pricing.map((item) => <span key={item}>{item}</span>)}</div>{tools.map((tool) => { const enabled = Boolean(status[tool.key]); return <article key={tool.key}><div><b>{tool.name}</b><span>{tool.detail}</span></div><button type="button" className={enabled ? "on" : "off"} disabled={saving === tool.key} onClick={() => toggle(tool.key)}>{saving === tool.key ? "Saving…" : enabled ? "Enabled" : "Paused"}</button></article>})}<div className="admin-note"><b>{message || "Before enabling paid tools"}</b><span>Enable a paid tool only after its provider billing and spend limit are verified.</span></div></section>;
}
