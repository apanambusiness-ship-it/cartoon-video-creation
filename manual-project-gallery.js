...
(()=>{
  ...
  panel.innerHTML=`<header class="apanam-gallery-header"><div><span class="apanam-gallery-eyebrow">APANAM CREATIVE STUDIO</span><h2>मेरा पोस्टर</h2><p>अपने डिज़ाइन सेव करें, खोलें और साझा करें।</p></div><button type="button" class="apanam-gallery-close" data-action="close" aria-label="गैलरी बंद करें">✕</button></header>
  ...
  async function render(){items.replaceChildren();const saved=(await operation('readonly','getAll')).sort((a,b)=>b.id-a.id);
    ...
    for(const entry of saved){const card=document.createElement('div');card.className='apanam-gallery-card';const preview=document.createElement('div');preview.className='apanam-gallery-preview';preview.append(thumb(entry));const title=document.createElement('strong');title.textContent=entry.name;title.title=entry.name;title.className='apanam-gallery-card-title';const open=document.createElement('button');open.type='button';open.textContent='खोलें';open.className='apanam-gallery-open';open.onclick=()=>{if(stage.querySelector('.element')&&!confirm('अभी खुला पोस्टर बदल जाएगा। क्या गैलरी वाला पोस्टर खोलें?'))return;if(project.restore(entry.project)){project.save();close()}};const rename=document.createElement('button');rename.type='button';rename.textContent='नाम बदलें';rename.className='apanam-gallery-rename';rename.onclick=async()=>{const name=prompt('पोस्टर का नया नाम लिखें',entry.name);if(name===null)return;const trimmed=name.trim().slice(0,80);if(!trimmed){status.textContent='पोस्टर का नाम खाली नहीं हो सकता।';return}try{await operation('readwrite','put',{...entry,name:trimmed});status.textContent='पोस्टर का नाम बदल गया।';await render()}catch(error){status.textContent='नाम सेव नहीं हुआ। फिर कोशिश करें।'}};const del=document.createElement('button');del.type='button';del.textContent='हटाएँ';del.className='apanam-gallery-delete';del.onclick=async()=>{if(!confirm('यह सेव किया हुआ पोस्टर गैलरी से हटाएँ?'))return;await operation('readwrite','delete',entry.id);await render()};const actions=document.createElement('div');actions.className='apanam-gallery-card-actions';actions.append(open,rename,del);card.append(preview,title,actions);items.append(card)}
  }
