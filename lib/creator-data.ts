...
export async function getCreatorProfile(email: string): Promise<CreatorProfile> {
  ...
  const requests = await sql`select id, status, language, prompt, created_at from creations where creator_email = ${email} and type = 'Manual custom design' order by created_at desc limit 12`;
  const profile = profiles[0];
  const kit = kits[0];
  return { plan: String(profile?.plan ?? "trial"), credits: Number(profile?.credits ?? 3), trialStartedAt: profile?.trial_started_at ? String(profile.trial_started_at) : null, brand: { brandName: String(kit?.brand_name ?? ""), tagline: String(kit?.tagline ?? ""), primaryColor: String(kit?.primary_color ?? "#634bc8"), accentColor: String(kit?.accent_color ?? "#ee6040") }, history: history.map((item) => ({ type: String(item.type), language: String(item.language), prompt: String(item.prompt), createdAt: String(item.created_at) })), requests: requests.map((item) => ({ id: String(item.id), status: String(item.status), language: String(item.language), prompt: String(item.prompt), createdAt: String(item.created_at) })) };
}
